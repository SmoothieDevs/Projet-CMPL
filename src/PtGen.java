...
public class PtGen {
 ...
 // MERCI de renseigner ici un nom pour le trinome, constitue EXCLUSIVEMENT DE LETTRES
    public static String trinome="William STEPHAN Evenn Resnais Badara TALL"; 
	...
	public static void pt(int numGen) {
		...
		case 1:
				//Lecture d'un Ident
			if(presentIdent(bc)==0 || tabSymb.length == 0) {
				if(bc == 0) {
					placeIdent(UtilLex.numIdCourant, VARGLOBALE, tCour, it++);	
				}else {
					placeIdent(UtilLex.numIdCourant, VARLOCALE, tCour, it++);
				}
			}else {
				tCour = tabSymb[presentIdent(bc)].type;
			}
